...
ParaLane structure (input):
    ...
    └── scene_X/
        └── clip_Y/
            ...
            ├── foreground_mask/{timestamp}/CAMERA_FRONT.png
...
DriveStudio structure (output):
    ...
    └── scene_X_clip_Y/
        ...
        ├── lidar/               {timestep:03d}.bin     (float32 Nx4: x,y,z,intensity, 1 bin file for each frame)
        ├── ego_pose/            {timestep:03d}.txt     (text file for each frame)
        ...
        ├── dynamic_masks/       {timestep:03d}_0.png   (coarse foreground mask)
            -- all (use same masks from input for all types of dynamic objects)
            -- human
            -- vehicle
...
Usage:
    python paralane_to_drivestudio_new.py \
...
def convert_clip(paralane_clip_dir, output_scene_dir, verbose=True):
    ...
    masks_dir = None
    for mask_name in ('foreground_mask', 'foreground_masks', 'foreground_labels', 'foreground_label'):
        candidate = clip_path / mask_name
        if candidate.exists():
            masks_dir = candidate
            break
    if masks_dir is None:
        masks_dir = clip_path / 'foreground_mask'
    ...
    # Keep intrinsics in the source image resolution.
    # CameraData infers actual image resolution from saved images and rescales intrinsics there.
    fx = K[0,0]
    fy = K[1,1]
    cx = K[0,2]
    cy = K[1,2]
    ...
    for timestep_idx, img_entry in enumerate(colmap_images):
        ...
        # Copy image → {timestep:03d}_0.jpg
        ...
        # Save ego_pose in a consistent frame.
        # Prefer lidar pose by timestamp; if unavailable, fall back to index-aligned lidar pose.
        ego_pose = pose_map.get(ts_part, None)
        if ego_pose is None and timestep_idx < len(lidar_poses):
        ...
        # Try matching mask to same timestamp directory using the ParaLane spec and
        # common naming variants (`foreground_mask` / `foreground_masks` / `foreground_labels`).
        ...
        if masks_dir.exists():
            for candidate in [
                ...
                masks_dir / ts_part / 'CAMERA_FRONT.png',
                masks_dir / ts_part / 'image.png',
            ]:
                if candidate.exists():
                    src_mask_path = candidate
                    break
            if src_mask_path is None:
                # Try matching by index when the mask directory contains timestamp subdirs.
                ...
                if timestep_idx < len(mask_dirs):
                    for candidate in [
                        ...
                        if candidate.exists():
                            src_mask_path = candidate
                            break
            if src_mask_path is None and masks_dir.exists():
                for candidate in sorted(masks_dir.glob('*')):
                    if candidate.is_file() and candidate.suffix.lower() in {'.png', '.jpg', '.jpeg'}:
                        if candidate.name.startswith('CAMERA_FRONT') or 'mask' in candidate.stem.lower():
                            src_mask_path = candidate
                            break
