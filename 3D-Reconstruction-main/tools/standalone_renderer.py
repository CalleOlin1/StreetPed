...
def _look_at_c2w(direction: torch.Tensor, position: torch.Tensor, ref_up: torch.Tensor) -> torch.Tensor:
...
def render_sky_supervision_novel_view(
    dataset,
    trainer,
    frame_index: int,
    lateral_offset_m: float,
    vertical_offset_m: float = 10.0,
):
    global _ROAD_MASK_WARNING_EMITTED
    pixel_source = dataset.pixel_source
    cam0 = pixel_source.camera_data[0]
    vertical_offset_m = 15
    ref_image_infos, ref_cam_infos = cam0.get_image(frame_index)
    ref_c2w = cam0.cam_to_worlds[frame_index].clone()
    ...
    novel_c2w[:3, 3] = (
        ...
        + vertical_offset_m * torch.tensor([0.0, 0.0, 1.0], device=ref_c2w.device, dtype=ref_c2w.dtype)
    )
    novel_c2w[:3, :3] = _look_at_c2w(
        direction=torch.tensor([0.0, 0.0, -1.0], device=ref_c2w.device, dtype=ref_c2w.dtype),
        position=novel_c2w[:3, 3],
        ref_up=ref_c2w[:3, 1],
    )[:3, :3]

    intrinsics = cam0.intrinsics[frame_index].clone()
    novel_cam_infos = {
    ...
    logger.info(
        f"Rendered one sky-supervision novel view from cam0 reference frame {frame_index} "
        f"with lateral offset {lateral_offset_m:.3f}m and vertical offset {vertical_offset_m:.3f}m"
    )
    return {
        "reference_frame_idx": frame_index,
        "rendered_rgb": rendered_rgb.cpu(),
        "background_rgb": background_rgb.detach().cpu() if isinstance(background_rgb, torch.Tensor) else None,
        "sky_rgb": sky_rgb.detach().cpu() if isinstance(sky_rgb, torch.Tensor) else None,
        "road_rgb": road_rgb.detach().cpu() if isinstance(road_rgb, torch.Tensor) else None,
...
def cli_render_sky_supervision_sample():
    parser = argparse.ArgumentParser(description="Render sky supervision sample (GPU isolated)")
    parser.add_argument("--checkpoint_path", type=str, required=True)
    parser.add_argument("--frame_index", type=int, required=True)
    parser.add_argument("--lateral_offset", type=float, required=True)
    parser.add_argument("--output_path", type=str, required=True)
    parser.add_argument("--vertical_offset", type=float, default=5.0)
    args = parser.parse_args()
    ...
    result = render_sky_supervision_novel_view(
        dataset,
        minimal_trainer,
        args.frame_index,
        -args.lateral_offset,
    ...
    print(f"Saved rendered sky-supervision sample to {args.output_path}")
...
if __name__ == "__main__":
    ...
    elif "render_sky_supervision_sample" in sys.argv:
        sys.argv.remove("render_sky_supervision_sample")
        cli_render_sky_supervision_sample()
